...
import br.com.beautystyle.retrofit.model.dto.BlockTimeDto;
...
public class BlockTimeRepository {
    ...
    public void updateAll(BlockTimeDto blockTimesFromApi, ResultsCallBack<BlockTimeDto> callBack) {
        getAllByDate().doOnSuccess(blockTimesFromRoom -> {
            mergeBlockTimeId(blockTimesFromApi, blockTimesFromRoom);
            insertAllOnRoom(blockTimesFromApi.getBlockTimesOnDay()).doOnSuccess(ids -> {
    ...
    private void setBlockTimeId(BlockTimeDto blockTimesFromApi,
                                List<Long> ids) {
        for (int i = 0; i < blockTimesFromApi.getBlockTimesOnDay().size(); i++) {
            blockTimesFromApi.getBlockTimesOnDay().get(i).setId(ids.get(i));
        }
    }

    private void mergeBlockTimeId(BlockTimeDto blockTimesFromApi, List<BlockTime> blockTimesFromRoom) {
        blockTimesFromRoom.forEach(fromRoom ->
                blockTimesFromApi.getBlockTimesOnDay().forEach(fromApi -> {
